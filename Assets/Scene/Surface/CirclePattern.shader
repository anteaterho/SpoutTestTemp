Shader "Hidden/ShaderToy/Voronoi"
{
	Properties
	{
	...
	float4 voronoi(in float2 x, float mode)
	{
		...
		for (int j = -2; j <= 2; j++)
			for (int i = -2; i <= 2; i++)
			{
				...
				// animate
				//o = 0.5 + 0.5*sin(_Time * 20.0 + 6.2831*o);
				o = sin(_Time);
